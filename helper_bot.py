...
def send_poster_to_channel(title, year, code, description=""):
    """Send poster and caption to channel"""
    try:
        ...
        if result.get('ok'):
            ...
            logging.error(f"Failed to send: {result}")
            return None
    except Exception as e:
...
def extract_movie_info(text):
    """Extract title, year, code, description"""
    lines = text.split('\n')
    title = None
    year = None
    code = None
            ...
            if 'forward_from_chat' in message:
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        send_message(chat_id, f"⚠️ Poster sent, but DB error: {resp.text}")
                except Exception as e:
                    send_message(chat_id, f"⚠️ Poster sent, but API error: {e}")
